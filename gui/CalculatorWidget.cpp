...
CalculatorWidget::CalculatorWidget(SettingsWidget* settings_widget, QWidget* parent)
    : QWidget(parent)
{
...
{
	connect(m_top_bar, &TopBarWidget::SettingsToggleRequested, this,
	        [this]()
	        {
		        m_settings_widget->IsOpen() ? m_settings_widget->Close()
		                                    : m_settings_widget->Open();
		        m_top_bar->raise();
	        });
	...
	connect(m_keypad, &KeypadWidget::EqualsPressed, this,
	        [this]() { emit EvaluateClicked(m_display->Text().toStdString()); });

	connect(m_keypad, &KeypadWidget::FunctionRequested, this,
	        [this](QString format_template)
	        {
	...
	connect(m_history_widget, &HistoryWidget::HistoryButtonClicked, this,
	        [this](QString result) { ShowResult(result.toDouble()); });
}
