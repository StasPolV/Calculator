...
CalculatorController::CalculatorController(CalculatorModel& model, CalculatorWidget& widget,
                                           QObject* parent)
    : QObject(parent), m_model(model), m_widget(widget)
{
	connect(&m_widget, &CalculatorWidget::EvaluateClicked, &m_model,
	...
	connect(&m_model, &CalculatorModel::EvaluatedError, &m_widget,
	        [this](const std::string& message)
	        { m_widget.ShowError(QString::fromStdString(message)); });
}
