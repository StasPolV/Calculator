...
KeypadWidget::KeypadWidget(QWidget* parent) : QWidget(parent)
{
	...
	for (const auto& info : buttons)
	{
		...
		else if (is_number)
		{
			connect(button, &QPushButton::clicked, this,
			        [this, info]() { emit DigitPressed(info.text); });
		}
		else
		{
			connect(button, &QPushButton::clicked, this,
			        [this, info]() { emit OperatorPressed(info.text); });
		}
	...
	connect(button_one_over_x, &QPushButton::clicked, this,
	        [this]() { emit FunctionRequested("1/(%1)"); });
	connect(button_power, &QPushButton::clicked, this,
	        [this]() { emit FunctionRequested("(%1)^2"); });
	connect(button_sqrt, &QPushButton::clicked, this,
	        [this]() { emit FunctionRequested("sqrt(%1)"); });
